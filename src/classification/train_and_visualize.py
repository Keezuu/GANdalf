...
def load_mixed_data():
    (x_train, y_train), (x_test, y_test) = utils.load_data(cnst.MNIST_IMGS_AMOUNT + 3000, 0.3)
    # We only want to load train images
    (xg_train, yg_train), (xg_test, yg_test) = utils.load_GAN_data(10000, 0.3)
...
def load_mixed_data_flat():
    (x_train, y_train), (x_test, y_test) = utils.load_data_flat(cnst.MNIST_IMGS_AMOUNT, cnst.VAL_SPLIT)
    (xg_train, yg_train), (xg_test, yg_test) = utils.load_GAN_data_flat(cnst.MNIST_IMGS_AMOUNT, 0)
    x_train = np.concatenate((x_train, xg_train))
    y_train = np.concatenate((y_train, yg_train))
    # Shuffling data
    sklearn.utils.shuffle(x_train, y_train)
...
def load_gan_train_mnist_val_data():
    (_, _), (x_test, y_test) = utils.load_data(cnst.MNIST_IMGS_AMOUNT, cnst.VAL_SPLIT)
    (xg_train, yg_train), (_, _) = utils.load_GAN_data(cnst.MNIST_IMGS_AMOUNT, 0)
    return (xg_train, yg_train), (x_test, y_test)


def load_gan_train_mnist_val_data_flat():
    (_, _), (x_test, y_test) = utils.load_data_flat(cnst.MNIST_IMGS_AMOUNT, cnst.VAL_SPLIT)
    (xg_train, yg_train), (_, _) = utils.load_GAN_data_flat(cnst.MNIST_IMGS_AMOUNT, cnst.VAL_SPLIT)
    return (xg_train, yg_train), (x_test, y_test)
...
def run_cnn():
    cnn = CNN()
    (x_train, y_train), (x_test, y_test) = utils.load_data(cnst.MNIST_IMGS_AMOUNT, cnst.VAL_SPLIT)
    history = cnn.train(x_train, y_train, x_test, y_test, cnst.EPOCHS)
    return history
...
def run_cnn_with_gan_data():
    ...
    history = cnn.train(x_train, y_train, x_test, y_test, cnst.EPOCHS_WITH_GAN)
    return history
